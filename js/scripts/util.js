...
const getNearbyEntities = (centerPos, range) => {
  ...
  GAME.entities.midground.forEach((entity) => {
    ...
    const closestX =
      leftDist < rightDist ? entity.boundingBox.left : entity.boundingBox.right;
    const closestY =
      topDist < bottomDist ? entity.boundingBox.top : entity.boundingBox.bottom;
  ...
  doubleTap: (key) => {
    ...
    switch (key.code) {
      case 'KeyD':
        ...
        console.log(
          'time since last D: ' + Math.round(GAME.lastKeyTime.keyD) + 's'
        );
      ...
      case 'KeyA':
        ...
        console.log(
          'time since last A: ' + Math.round(GAME.lastKeyTime.keyA) + 's'
        );
...
const takeCanvasScreenshot = () => {
  ...
  link.download = `gbc_screenshot_${Date.now() % 100_000}`;
  link.click();
};
