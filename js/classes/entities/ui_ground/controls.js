...
class Controls {
    ...
    static get POSITION() {
        return Vector.multiply(
            Vector.subtract(Camera.SIZE, Controls.SIZE),
            1 / 2
        );
    }
    ...
    static get SIZE() {
        return new Vector(
            500,
            Controls.CONTROLS.length * Controls.FONT_SIZE + 3 * Controls.PADDING
        );
    }
    ...
    draw() {
        ...
        CTX.fillText(
            'Controls',
            (Camera.SIZE.x - headerSize.width) / 2,
            pos.y - Controls.HEADER_MARGIN
        );

        const textStart = new Vector(
            pos.x + Controls.PADDING,
            pos.y + Controls.PADDING + Controls.FONT_SIZE
        );
        // draw the text
        ...
        for (let i = 0; i < Controls.CONTROLS.length; i++) {
            CTX.fillText(
                Controls.CONTROLS[i],
                textStart.x,
                textStart.y + Controls.FONT_SIZE * i
            );
        }
    }
}
